...
def ident_pivot_nodes(adjs, weights, thres):
    ...
    N = adjs[0].shape[0]
    T = len(weights)
    total_inta_mat = coo_matrix((N, T))
    total_inta_mat = total_inta_mat.tolil()
    for t, adj in enumerate(adjs):
        total_inta_mat[:, t] = np.array(adj.sum(axis=1).reshape(-1)).reshape((N, 1))
    total_inta_mat = csc_matrix(np.dot(total_inta_mat.todense(), weights))
    total_inta_mat /= total_inta_mat.max()
    pivot_nodes, _ = np.where(total_inta_mat.todense() > thres)
